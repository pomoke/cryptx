// AES-CTR based.
use crate::{aes::{Aes, self}, common::CryptError};
...
impl AesCtrHmac {
    ...
    pub fn encrypt_stream(&mut self, data: &[u8]) -> Vec<u8> {
        ...
        // Pad and slice data.
        let append_zero_count = if data.len() % 16 != 0 {16 - (data.len() % 16)} else {0};
    ...
    pub fn decrypt_raw(&self,msg: &[u8]) -> Result<Vec<u8>,CryptError> {
        ...
        for i in 0..(msg.len()/16) {
            let block = Aes::decrypt::<9,11>(&msg[(i*16)..(i*16+16)].try_into().unwrap(), &self.key_schedule);
