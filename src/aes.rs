...
const RoundConst : [u32;14]= [0,0x1,0x2,0x4,0x8,0x10,0x20,0x40,0x80,0x1b,0x36,0,0,0];
    ...
    fn aes128_key_schedule(key: [u8;16]) -> [[u8;16];11] {
        ...
        for i in 0..N*Rounds {
            if i < N {
                ...
                words[i] = words[i-N] ^ (Self::sub_word32(Self::rot_word32(words[i-1]))) ^ RoundConst[i / N];
            }
        ...
        for i in 0..Rounds {
            for j in 0..4 {
                ret[i][4*j..4*j+4].copy_from_slice(word_to_bytes(words[4*i+j]).as_mut_slice());
            }
        }
        ret
    }
