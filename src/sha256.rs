...
// From FIPS PUB 180-2.
use crate::common::{CryptoHash,CryptError};
...
impl SHA256 {
    ...
    fn process(data: &[u8]) -> [u8;32] {
        ...
        for i in 0..(data.len() / 64) {
            ...
            for j in 0..16 {
                let conv_from = [data[64*i+4*j+0],data[64*i+4*j+1],data[64*i+4*j+2],data[64*i+4*j+3]];
            ...
            for j in 16..64 {
                ...
                let Wrapping(w_j) = Wrapping(w[j-16]) + Wrapping(s0) + Wrapping(w[j-7]) + Wrapping(s1);
                w[j] = w_j
            ...
            for j in 0..64 {
                ...
                let ch = (e & f) ^ ((!e) & g);
                let Wrapping(temp1) = Wrapping(h) + Wrapping(s1) + Wrapping(ch) + Wrapping(K[j]) + Wrapping(w[j]);
...
fn sha256_test() {
    ...
    println!("{}",result_str);
    assert_eq!(result,<[u8;32]>::from_hex("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad").unwrap());
    let result = SHA256::hash(vec![].as_ref()).unwrap();
    assert_eq!(result,<[u8;32]>::from_hex("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855").unwrap());
}
